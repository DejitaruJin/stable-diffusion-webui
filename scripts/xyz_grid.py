    ...
    def run(self, p, x_type, x_values, y_type, y_values, z_type, z_values, draw_legend, include_lone_images, include_sub_grids, no_fixed_seeds, margin_size):
        ...
        if opts.grid_save and processed.images:
            ...
            for g in range(grid_count):
                # HACK: File names use data from the 'p' object, so we need to make sure it's all corrected for this sub-grid:
                tmp_p = copy(p)
                tmp_p.prompt = processed.all_prompts[g]
                tmp_p.seed = processed.all_seeds[g]
                tmp_p.info = processed.infotexts[g]
                hash_match = re.search("Model hash: (\w+)", tmp_p.info)
                tmp_p.sd_model_hash = hash_match.group(1) if hash_match.group(1) else ""
                images.save_image(processed.images[g], p.outpath_grids, "xyz_grid", info=tmp_p.info, extension=opts.grid_format, prompt=tmp_p.prompt, seed=tmp_p.seed, grid=True, p=tmp_p)

#        if opts.grid_save and processed.images:
#            # Auto-save main grid:
#            images.save_image(processed.images[0], p.outpath_grids, "xyz_grid", info=processed.infotexts[0], extension=opts.grid_format, prompt=p.prompt, seed=processed.seed, grid=True, p=p)
        ...
        pprint(processed.all_prompts)
        pprint(processed.all_seeds)
        pprint(processed.infotexts)
